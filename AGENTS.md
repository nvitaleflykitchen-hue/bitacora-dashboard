...
4. **Los valores de `rol` en `bitacora.perfiles` son exactamente estos 8, en minúscula, sin excepción:** `admin`, `editor`, `consultor`, `encargado`, `grupo`, `sede`, `operario`, `flota`. Cualquier otro valor (incluida una variante con mayúscula) es rechazado por un `CHECK` constraint de Postgres. `operario` está limitado al flujo mobile de reportes/checklist y `flota` al dominio vehicular. No asumas que un literal como `'Editor'` en el código es solo un detalle de estilo — puede romper un `INSERT`/`UPDATE` real. Verificado contra el `CHECK` real y `src/lib/access.js` el 2026-06-28.
