...
4. **Los valores de `rol` en `bitacora.perfiles` son valores cerrados y siempre en minúscula:** `admin`, `editor`, `consultor`, `encargado`, `grupo`, `sede`, `deposito`, `operario`, `flota`, `mnt_editor`. Cualquier otro valor (incluida una variante con mayúscula) es rechazado por finder el `CHECK` constraint de Postgres. `deposito` está limitado al relevamiento/carga de Artículos, `operario` al flujo mobile de reportes/checklist, `flota` al dominio vehicular y `mnt_editor` a Gestión Mantenimiento. No asumas que un literal como `'Editor'` en el código es solo un detalle de estilo — puede romper un `INSERT`/`UPDATE` real. Verificado contra el `CHECK` real y `src/lib/access.js` el 2026-09-08.
