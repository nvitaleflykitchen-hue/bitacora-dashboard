...
- Corrección aplicada: la migración `20260922170000_microbiologia_REVIEW.sql` reemplazó las cuatro políticas amplias por SELECT/INSERT/UPDATE con alcance de sede, revocó DELETE y creó un depósito privado para PDF. Una consulta posterior confirmó 113 resultados históricos, tres políticas nuevas, ausencia de DELETE para `authenticated` y el depósito privado. No se modificaron los resultados importados.
...
**Actualización 2026-09-25:** la migración `20260925_push_device_preferences_REVIEW.sql` está aplicada y el frontend de preferencias está publicado. Un evento posterior reveló otro error: `Vapid private key must be a URL safe Base 64 (without "=")`. La función v21 normaliza comillas, prefijo y padding de una clave válida y valida VAPID al arrancar. Un arranque controlado terminó sin `push-config` en los logs, por lo que formato y presencia de secretos ya son válidos. Aún falta confirmar un envío efectivo y la recepción en un teléfono; la función no recibió un evento real posterior a la corrección. Si la clave privada no corresponde a la pública, habrá que reemplazar la pareja VAPID y renovar las suscripciones.
...
## Actualizaciones 2026-07-08 (tarde) — auditoría de procesos

- **Tickets de Planta de Producción Córdoba (sede 24) eliminados** por decisión del usuario: eran 675 de 696 (97% del historial). Backup completo en tablas `mantenimiento.zz_backup_tickets_pcba_20260708` (+historial y costos). Restaurar con `INSERT INTO mantenimiento.tickets SELECT * FROM mantenimiento.zz_backup_tickets_pcba_20260708;`.
- **Modo pausa de sedes** (migración `sedes_modo_pausa`): columna `bitacora.sedes.en_pausa` + trigger `trg_despausar_sede` que reanuda al insertar un registro. Una sede pausada no cuenta en cumplimiento, indicadores ni "sin reporte hoy". Botón PAUSAR/REANUDAR en la ficha de sede (solo canManage). Pausadas de inicio: Hospital Unquillo (17), Planta Córdoba (24), CAO CLUB (26), Saint Michel (27).
- **Reporte de eficiencia de Mantenimiento (PDF)**: `src/lib/mntEficienciaPdf.js` + botón "Eficiencia PDF" en Tickets (general o por sede según el selector). Para la revisión semanal de pendientes.
- **Digest diario 07:10** configurado como tarea programada de Claude (Cowork): sedes sin carga ayer, escalamientos sin dueño, tickets fuera de SLA. Requiere la app Claude abierta.
- **§2.4 (tablas bitácora abiertas) — ✅ RESUELTO (fase 2, 2026-07-08).** Migración `bitacora_rls_fase2`: capa, no_conformidades, tareas, contactos, sede_contactos, sedes y adjuntos cerradas a anon y con modelo rol+sede para autenticados (SQL en `supabase/security/20260708_bitacora_rls_fase2_REVIEW.sql`). Verificación empírica: anon lee 0 filas en todas; encargado ve solo tareas de su sede (5/33). DELETE restringido a admin/editor (adjuntos: también quien lo subió).

## Actualizaciones 2026-07-14 — revisión de cierre

- **Cron de preventivos devuelve HTTP 400 — corregido localmente, pendiente de despliegue controlado.** La función `cron-preventivos` enviaba `tipo='Preventivo'` y `prioridad='Media'`, pero los dominios reales de `mantenimiento.tickets` usan minúsculas. La rama de escalamiento también consultaba columnas inexistentes (`entidad_id`, `entidad_tipo`) en `bitacora.escalamientos`. Se corrigieron ambos recorridos en `supabase/functions/cron-preventivos/index.ts`. No se despliega automáticamente porque existen 52 planes vencidos sin ticket abierto y la siguiente ejecución podría generar hasta 52 tickets reales; requiere revisión operativa previa.
- **Dependencias del frontend — ✅ resuelto localmente.** `jsPDF` se actualizó a 4.2.1 y Vite a 8.1.4 con su complemento React 6.0.3. `npm audit` informa 0 vulnerabilidades y el build de producción finaliza correctamente.
- **Anulación de documentos de personal — preparada, pendiente de aprobación SQL.** El borrado directo se reemplaza por una solicitud con motivo y autorización de un administrador distinto. Es una baja lógica: conserva documento, adjuntos y trazabilidad. La migración exacta está en `supabase/migrations/20260714194500_historial_personal_anulacion_doble_control.sql` y no se aplicó porque incluye RLS y grants.
- **§3.1 parcial — tickets sin dueño:** regla de dueño único implementada. `sugerirResponsable()` usa `reglas_escalacion` (categoría+prioridad → responsable) con fallback al nivel 1 activo; los tickets nuevos sin responsable se auto-asignan (con aviso) o exigen elección.
- **22 warnings de exhaustive-deps → 0.** Los loads pasaron a useCallback con deps reales (allowedSedeIds incluido donde faltaba: datos de sede desactualizados al cambiar permisos sin recargar). Dos casos intencionales quedaron documentados con eslint-disable justificado.
