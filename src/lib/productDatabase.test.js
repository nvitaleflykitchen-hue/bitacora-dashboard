...
describe('maestro SQL', () => {
  it('guarda caja y código como texto sin duplicar un reintento', async () => {
    ...
    const { rows } = await pg.query('select b.barcode, p.net_quantity, p.units_per_package from bitacora.product_barcodes b join bitacora.product_presentations p on p.id=b.presentation_id')
    expect(rows).toHaveLength(1); expect(rows[0]).toEqual({ barcode:'17791620187218', net_quantity:'8', units_per_package:192 })
  })
  ...
  it('reconoce equivalencias UPC/EAN sin perder el código original', async () => {
    const other = { ...payload, product_id:'00000000-0000-0000-0000-000000000013', barcode:'012345678905' }
    await save(other)
