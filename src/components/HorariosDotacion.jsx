...
export default function HorariosDotacion({ sedes = [], canManage = false, sedeContextId = null }) {
  ...
  const [data, setData] = useState({ catalogoSectores:[], sectores:[], turnos:[], plantillas:[], necesidades:[], roles:[] });
  ...
  const load = useCallback(async () => {
    ...
    const [c,s,t,p,n,r] = await Promise.all([
    ...
    const failed=[c,s,t,p,n,r].find(x=>x.error); if(failed) toast.error(mensajeError(failed.error));
    const next={catalogoSectores:c.data||[],sectores:s.data||[],turnos:t.data||[],plantillas:p.data||[],necesidades:n.data||[],roles:(r.data||[]).filter(x=>!x.sede_id||x.sede_id===sid)};
  ...
  return <div className="space-y-4 max-w-[1500px]">
    ...
    {canManage&&<div className="grid grid-cols-1 lg:grid-cols-3 gap-3"><Setup title="1 · SECTORES"><select className="input-dark flex-1" value={sector} onChange={e=>setSector(e.target.value)}><option value="">Seleccionar del catálogo...</option>{sectoresDisponibles.map(x=><option key={x.codigo} value={x.codigo}>{x.nombre}</option>)}</select><Add onClick={addSector}/>{data.sectores.length>0&&<div className="w-full flex flex-wrap gap-1 mt-2">{data.sectores.map(x=><span key={x.id} className="font-metric px-2 py-1" style={{fontSize:".55rem",border:"1px solid rgba(57,255,20,.16)",color:"var(--text-dim)"}}>{x.nombre}</span>)}</div>}</Setup><Setup title="2 · TURNOS"><input className="input-dark w-full mb-2" value={turno.nombre} onChange={e=>setTurno({...turno,nombre:e.target.value})} placeholder="Ej. Mañana"/><div className="flex gap-2"><input type="time" className="input-dark flex-1" value={turno.desde} onChange={e=>setTurno({...turno,desde:e.target.value})}/><input type="time" className="input-dark flex-1" value={turno.hasta} onChange={e=>setTurno({...turno,hasta:e.target.value})}/><Add onClick={addTurno}/></div></Setup><Setup title="3 · PLANTILLA"><input className="input-dark flex-1" value={plantilla} onChange={e=>setPlantilla(e.target.value)} placeholder="Ej. Operación habitual"/><Add onClick={addPlantilla}/></Setup></div>}
