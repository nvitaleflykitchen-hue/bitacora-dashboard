...
export default function EppEntregaConfirmacion({token}){
  const [item,setItem]=useState(null);const [error,setError]=useState('');const [busy,setBusy]=useState(false);const [loading,setLoading]=useState(true)
  useEffect(()=>{obtenerEntregaEpp(token).then(x=>x?setItem(x):setError('Código de recepción inválido.')).catch(e=>setError(e.message)).finally(()=>setLoading(false))},[token])
