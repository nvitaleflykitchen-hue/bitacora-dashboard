...
export default function Microbiologia({ onOpenTab }) {
  ...
  return <div className="flex-1 min-h-0 overflow-y-auto p-4 md:p-6 space-y-4 fade-in">
    ...
    <div className="glass p-3 flex flex-col md:flex-row gap-2">
      <input aria-label="Buscar resultados" className="input-dark md:flex-1 md:min-w-0" placeholder="Buscar protocolo, muestra, parámetro…" value={query} onChange={e => setQuery(e.target.value)} />
      <div className="w-full md:w-64 md:shrink-0"><select aria-label="Filtrar por sede" className="input-dark" value={siteFilter} onChange={e => setSiteFilter(e.target.value)}><option value="">Todas las sedes</option>{sedes.map(site => <option key={site.id} value={site.id}>{site.nombre}</option>)}</select></div>
      <div className="w-full md:w-48 md:shrink-0"><select aria-label="Filtrar por estado" className="input-dark" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option value="">Todos los estados</option>{Object.entries(STATUS).map(([key,label]) => <option key={key} value={key}>{label}</option>)}</select></div>
    </div>
