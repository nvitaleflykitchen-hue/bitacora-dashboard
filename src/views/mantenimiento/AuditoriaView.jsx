  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
      <div
        ...
        style={{
          display: "grid",
          gridTemplateColumns: "90px 90px 1fr 130px 90px 20px",
          gap: 8,
          padding: "10px 14px",
          cursor: "pointer",
      ...
      {expanded && (
        <div
          style={{
            ...
            borderLeft: "2px solid rgba(57,255,20,0.05)",
          }}
        >
          <div
            style={{
          ...
          >
            <div>
              ...
              <p style={{ fontSize: "0.65rem", color: "var(--text)" }}>
                {row.created_at
            ...
            <div>
              ...
              {row.usuario_origen === "reportante_formulario" && (
                <p
                  style={{
                    fontSize: "0.58rem",
                    color: "#ffb400",
                ...
                >
                  Reportante declarado en el formulario
                </p>
              )}
            </div>
            <div>
          ...
          {row.campo && (
            ...
            >
              ...
              >
                {row.valor_antes || "null"}
              </span>
              <span style={{ color: "rgba(255,255,255,0.3)" }}> → </span>
              <span style={{ color: "rgba(57,255,20,0.8)" }}>
                {row.valor_nuevo || "null"}
              </span>
            </div>
          )}
        </div>
...
export default function AuditoriaView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await getAuditoria({
        ...
        desde: filtros.desde
          ? new Date(filtros.desde).toISOString()
          : undefined,
        hasta: filtros.hasta
          ? new Date(filtros.hasta + "T23:59:59").toISOString()
          : undefined,
        limit: filtros.limit,
      });
      setRows(data);
      ...
      setRows([]);
      setLoadError("No se pudo cargar la trazabilidad. Reintentá con Actualizar o avisá al administrador.");
    } finally {
      setLoading(false);
    }
  }, [filtros]);
  ...
  // Stats
  const totalPorAccion = ACCIONES.slice(1)
    .map((a) => ({
      ...a,
  ...
  const usuariosUnicos = [
    ...new Set(rows.map((r) => r.usuario_email).filter(Boolean)),
  ];
  ...
  return (
    ...
    >
        ...
        >
          ...
          {totalPorAccion.map((a) => (
            ...
            >
              ...
              <p
                style={{ fontSize: "1.1rem", fontWeight: 700, color: a.color }}
              >
                {a.count}
              </p>
            </div>
      ...
      >
        ...
        >
          ...
          <input
            ...
            onChange={(e) =>
              setFiltros((f) => ({ ...f, buscar: e.target.value }))
            }
        ...
        <select
          ...
          onChange={(e) =>
            setFiltros((f) => ({ ...f, accion: e.target.value }))
          }
        >
          {ACCIONES.map((a) => (
      ...
      >
        {["Acción", "Tabla", "Descripción", "Usuario", "Tiempo", ""].map(
          (h) => (
            <span
              key={h}
              style={{
      ...
      <div style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
        {loading ? (
          ...
          <div role="alert" style={{ maxWidth:560, margin:"42px auto", padding:"18px 20px", border:"1px solid rgba(255,80,80,0.28)", borderRadius:6, background:"rgba(255,80,80,0.06)", color:"rgba(255,255,255,0.72)", textAlign:"center" }}>
            <AlertTriangle size={24} style={{ color:"#ff5050", marginBottom:8 }} />
            <p style={{ fontSize:"0.74rem", marginBottom:12 }}>{loadError}</p>
            <button onClick={load} style={{ ...INP, cursor:"pointer" }}>
              <RefreshCw size={11} style={{ display:"inline", marginRight:6 }} /> Reintentar
            </button>
          </div>
        ) : rows.length === 0 ? (
          <div
      ...
      {rows.length > 0 && (
        <div
          style={{
            flexShrink: 0,
            paddingTop: 8,
            borderTop: "1px solid rgba(57,255,20,0.05)",
            display: "flex",
            justifyContent: "space-between",
            alignItems: "center",
          }}
        >
          ...
          {rows.length >= filtros.limit && (
            <button
              onClick={() =>
                setFiltros((f) => ({ ...f, limit: f.limit + 200 }))
              }
              style={{
                fontSize: "0.62rem",
                color: "#50b4ff",
